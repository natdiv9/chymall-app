...
export class RetraitsDemandeNewComponent implements OnInit {
    ...
    newDemandeRetrait(content: any, c: any) {
        const demande = {
            ...
            nom: this.demandeRetraitForm.get('nom').value,
            prenom: this.demandeRetraitForm.get('prenom').value,
            frais_retrait: this.frais_retrait,
            montant_remis: this.montant_remis,
            operateur_transfert: this.authService.currentUser.username,
    ...
    calculer_frais_retrait(value: number) {
        this.frais_retrait = (value * 5 / 100);
        this.montant_remis = value - this.frais_retrait;
    }
}
